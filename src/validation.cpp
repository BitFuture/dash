...
namespace {
    ...
    CBlockIndex *pindexBestInvalid;//最后一个无效的块
...
bool CheckTransaction(const CTransaction& tx, CValidationState &state)
{
    ...
    if (tx.vin.empty())//输入为空
        return state.DoS(10, false, REJECT_INVALID, "bad-txns-vin-empty");
    if (tx.vout.empty())//输出为空
        return state.DoS(10, false, REJECT_INVALID, "bad-txns-vout-empty");
    // Size limits
    if (::GetSerializeSize(tx, SER_NETWORK, PROTOCOL_VERSION) > MAX_LEGACY_BLOCK_SIZE)//数据太大
    ...
    BOOST_FOREACH(const CTxOut& txout, tx.vout)
    {
        if (txout.nValue < 0)//没有输出
            return state.DoS(100, false, REJECT_INVALID, "bad-txns-vout-negative");
        if (txout.nValue > MAX_MONEY)//输出太大
            return state.DoS(100, false, REJECT_INVALID, "bad-txns-vout-toolarge");
        nValueOut += txout.nValue;
        if (!MoneyRange(nValueOut))//总输出太大
    ...
    {
        BOOST_FOREACH(const CTxIn& txin, tx.vin)//任何输入，必须有来源
                              ...
                              std::vector<COutPoint>& coins_to_uncache, bool fDryRun)
{
    ...
    //CheckTransaction 检查输入合法，输入的　Preout　合法，检查输出合法
    //ContextualCheckTransaction 检查大小　根据版本检查大小
    if (!CheckTransaction(tx, state) || !ContextualCheckTransaction(tx, state, chainActive.Tip()))
        return false;

    // Coinbase is only valid in a block, not as a loose transaction
    // 矿工奖励交易不需要加入　mempool，也不需要广播
    if (tx.IsCoinBase())
        return state.DoS(100, false, REJECT_INVALID, "coinbase");

    // Rather not work on nonstandard transactions (unless -testnet/-regtest)
    string reason;  //是不是标准数据　或者输出签名不正确
...
void CheckForkWarningConditions()
{
    ...
    if (pindexBestForkTip && chainActive.Height() - pindexBestForkTip->nHeight >= 72)
        pindexBestForkTip = NULL;//没有最后分叉的顶部
    //pindexBestInvalid 最后一个不合法的块
    if (pindexBestForkTip || (pindexBestInvalid && pindexBestInvalid->nChainWork > chainActive.Tip()->nChainWork + (GetBlockProof(*chainActive.Tip()) * 6)))
    {   //最后分叉的顶部，或者最大不合法大于6个合法顶部
        ...
        if (pindexBestForkTip && pindexBestForkBase)
        {
            if(pindexBestForkBase->phashBlock){
                ...
                fLargeWorkForkFound = true;//最大分叉找到了
            }
        }
        else
        {
            if(pindexBestInvalid->nHeight > chainActive.Height() + 6)//丢了6个
...
void CheckForkWarningConditionsOnNewFork(CBlockIndex* pindexNewForkTip)
{
    ...
    CBlockIndex* pfork = pindexNewForkTip;//最后一个组中，最小的一个块，可能部分已经加进去了，所以比 plonger 小
    CBlockIndex* plonger = chainActive.Tip();//顶部
    while (pfork && pfork != plonger) //最后添加的，和顶部找到中间相同的
    {
    ...
    {
        pindexBestForkTip = pindexNewForkTip;//最后分叉的顶部
        pindexBestForkBase = pfork;//最后添加的分叉块
    }
...
void static InvalidChainFound(CBlockIndex* pindexNew)
{
    if (!pindexBestInvalid || pindexNew->nChainWork > pindexBestInvalid->nChainWork)
        pindexBestInvalid = pindexNew;//最后一个无效的

    LogPrintf("%s: invalid block=%s  height=%d  log2_work=%.8g  date=%s\n", __func__,
      pindexNew->GetBlockHash().ToString(), pindexNew->nHeight,
      log(pindexNew->nChainWork.getdouble())/log(2.0), DateTimeStrFormat("%Y-%m-%d %H:%M:%S",
      pindexNew->GetBlockTime()));
    CBlockIndex *tip = chainActive.Tip();//顶部
    assert (tip);
    LogPrintf("%s:  current best=%s  height=%d  log2_work=%.8g  date=%s\n", __func__,
      tip->GetBlockHash().ToString(), chainActive.Height(), log(tip->nChainWork.getdouble())/log(2.0),
      DateTimeStrFormat("%Y-%m-%d %H:%M:%S", tip->GetBlockTime()));
    CheckForkWarningConditions();//检查
}
...
static bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck = false)
{
    ...
    // (its coinbase is unspendable) 旷世区块
    ...
    if (!hashAssumeValid.IsNull()) {//最后一个检查的块，系统设置好的 
    ...
    if (fEnforceBIP30) {
        BOOST_FOREACH(const CTransaction& tx, block.vtx) {
            for (size_t o = 0; o < tx.vout.size(); o++) {
                if (view.HaveCoin(COutPoint(tx.GetHash(), o))) {//如果已经存在一个交易，就退出了
    ...
    CCheckQueueControl<CScriptCheck> control(fScriptChecks && nScriptCheckThreads ? &scriptcheckqueue : NULL);//签名检查
    ...
    for (unsigned int i = 0; i < block.vtx.size(); i++)
    {
        ...
        if (nSigOps > MaxBlockSigOps(fDIP0001Active_context))//签名个数太多了
            return state.DoS(100, error("ConnectBlock(): too many sigops"),
                             REJECT_INVALID, "bad-blk-sigops");

        if (!tx.IsCoinBase())
        {
            if (!view.HaveInputs(tx))//查找input的out是否存在
            ...
            prevheights.resize(tx.vin.size());//入的前一个的高度列表
            ...
            control.Add(vChecks);//这个地方检查交易的合法性
        }
    ...
    if (pindex->GetUndoPos().IsNull() || !pindex->IsValid(BLOCK_VALID_SCRIPTS))//签名不合法
    {
    ...
    static uint256 hashPrevBestCoinBase; //更新
...
bool static ConnectTip(CValidationState& state, const CChainParams& chainparams, CBlockIndex* pindexNew, const CBlock* pblock)
{
    ...
    // to conflicted:
    // 接受到块后，某些交易已经加到块中了，也就是可能消费了，所以需要刷新钱包    
    //通知 刷新接受到，交易池中存在的交易，一般都是钱包响应
    BOOST_FOREACH(const CTransaction &tx, txConflicted) {
        GetMainSignals().SyncTransaction(tx, NULL);
    }
    // ... and about transactions that got confirmed:
    //通知刷新本块交易，一般都是钱包响应
    // 接受到块有，可能钱包增加了交易，也就是别人给自己了，所以需要刷新钱包
...
static bool ActivateBestChainStep(CValidationState& state, const CChainParams& chainparams, CBlockIndex* pindexMostWork, const CBlock* pblock, bool& fInvalidFound)
{
    ...
    bool fContinue = true;
    int nHeight = pindexFork ? pindexFork->nHeight : -1;//分叉的位置
    while (fContinue && nHeight != pindexMostWork->nHeight) {
        // Don't iterate the entire list of potential improvements toward the best tip, as we likely only need
        // a few blocks along the way.
        int nTargetHeight = std::min(nHeight + 32, pindexMostWork->nHeight);//每次最多玩32个块
        vpindexToConnect.clear();
        vpindexToConnect.reserve(nTargetHeight - nHeight);
        CBlockIndex *pindexIter = pindexMostWork->GetAncestor(nTargetHeight);//找到这个高度的块
        while (pindexIter && pindexIter->nHeight != nHeight) {//把找到高度之前的所有块都链进来 nTargetHeight 这个是目标
            vpindexToConnect.push_back(pindexIter);//小块在后面
        ...
        BOOST_REVERSE_FOREACH(CBlockIndex *pindexConnect, vpindexToConnect) {//重新连接所有块
            if (!ConnectTip(state, chainparams, pindexConnect, pindexConnect == pindexMostWork ? pblock : NULL)) {
                if (state.IsInvalid()) {
                    ...
                    if (!state.CorruptionPossible())//赋值最优一个坏的块
...
CBlockIndex* AddToBlockIndex(const CBlockHeader& block)
{
    ...
    BlockMap::iterator it = mapBlockIndex.find(hash);//如果已经找到 
    if (it != mapBlockIndex.end())
        return it->second;
    ...
    BlockMap::iterator miPrev = mapBlockIndex.find(block.hashPrevBlock);//找到前一个
...
bool CheckBlockHeader(const CBlockHeader& block, CValidationState& state, bool fCheckPOW)
{
    ...
    // Check proof of work matches claimed amount  检查算力保证
    if (fCheckPOW && !CheckProofOfWork(block.GetHash(), block.nBits, Params().GetConsensus()))
        return state.DoS(50, error("CheckBlockHeader(): proof of work failed"),
                         REJECT_INVALID, "high-hash");

    // Check timestamp　检查时间戳
...
bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, CBlockIndex * const pindexPrev)
{
    ...
    int nHeight = pindexPrev->nHeight + 1;
    // Check proof of work
    if(Params().NetworkIDString() == CBaseChainParams::MAIN && nHeight <= 68589){  //检查工作量必须合法　小于某个区块的时候，在某个范围即可　？？？？？？
    ...
    if (block.GetBlockTime() <= pindexPrev->GetMedianTimePast())//检查时间戳，必须比前面的时间戳大
        return state.Invalid(error("%s: block's timestamp is too early", __func__),
                             REJECT_INVALID, "time-too-old");

    // Reject block.nVersion=1 blocks when 95% (75% on testnet) of the network has upgraded:
    // IsSuperMajority 从　pindexPrev　往前数　consensusParams.nMajorityWindow　１０００　个，当版本号大于２　的个数超过　consensusParams.nMajorityRejectBlockOutdated　９５０　认为版本号不合法
...
static bool AcceptBlockHeader(const CBlockHeader& block, CValidationState& state, const CChainParams& chainparams, CBlockIndex** ppindex)
{
    ...
    // Check for duplicate
    uint256 hash = block.GetHash();
    BlockMap::iterator miSelf = mapBlockIndex.find(hash);//从现有的块中找是否存在接受到的块
    CBlockIndex *pindex = NULL;

    // TODO : ENABLE BLOCK CACHE IN SPECIFIC CASES
    if (hash != chainparams.GetConsensus().hashGenesisBlock) {//如果不是创世区块

        if (miSelf != mapBlockIndex.end()) {  //如果找到了
            // Block header is already known.
            pindex = miSelf->second;
            if (ppindex)
                *ppindex = pindex;
            if (pindex->nStatus & BLOCK_FAILED_MASK) //如果找到的块头有问题，返回失败
                return state.Invalid(error("%s: block is marked invalid", __func__), 0, "duplicate");
            return true;//返回找到的块头
        }
        //两个检查头部　CheckProofOfWork　检查挖矿算力合法
        //检查块的时间　小于当前时间　－　２　小时范围内，
        if (!CheckBlockHeader(block, state))
            return false;

        // Get prev block index
        CBlockIndex* pindexPrev = NULL;
        BlockMap::iterator mi = mapBlockIndex.find(block.hashPrevBlock);//找到前一个块
        if (mi == mapBlockIndex.end())//没有找到，认为不合法
            return state.DoS(10, error("%s: prev block not found", __func__), 0, "bad-prevblk");
        pindexPrev = (*mi).second;
        if (pindexPrev->nStatus & BLOCK_FAILED_MASK)//前一个块有问题
            return state.DoS(100, error("%s: prev block invalid", __func__), REJECT_INVALID, "bad-prevblk");

        assert(pindexPrev);//根据checkpoint数据检查高度，这个有疑问。？？？？？？？
...
{//接受到块信息，或者挖矿产生新块
    ...
    bool fTooFarAhead = (pindex->nHeight > int(chainActive.Height() + MIN_BLOCKS_TO_KEEP));//不要离得太远了 288
    ...
    if ((!CheckBlock(block, state)) || !ContextualCheckBlock(block, state, pindex->pprev)) {
        if (state.IsInvalid() && !state.CorruptionPossible()) {
            pindex->nStatus |= BLOCK_FAILED_VALID;
            setDirtyBlockIndex.insert(pindex);//AddToBlockIndex 中已经加过一次
        }
        return false;
    }
...
{
    AssertLockHeld(cs_main);
    assert(pindexPrev && pindexPrev == chainActive.Tip()); //前一个块必须是 最高块
...
{
    const CChainParams& chainparams = Params();
    if (!pblocktree->LoadBlockIndexGuts(InsertBlockIndex))//读取所有块头信息
        return false;
    ...
    vSortedByHeight.reserve(mapBlockIndex.size());//排序
    BOOST_FOREACH(const PAIRTYPE(uint256, CBlockIndex*)& item, mapBlockIndex)
    {
        CBlockIndex* pindex = item.second;
        vSortedByHeight.push_back(make_pair(pindex->nHeight, pindex));
    }
    sort(vSortedByHeight.begin(), vSortedByHeight.end());//从新计算难度系数总和
    ...
    bool fReindexing = false;//是否需要重构index
    pblocktree->ReadReindexing(fReindexing);
    fReindex |= fReindexing;
    //InitBlockIndex 以下参数都是 这个函数初始化的时候写入的，也就是第一次写入有效
...
bool InitBlockIndex(const CChainParams& chainparams) 
{
    ...
    if (!fReindex) {
        try {
            CBlock &block = const_cast<CBlock&>(chainparams.GenesisBlock());  //取得创世区块
            // Start new block file
            unsigned int nBlockSize = ::GetSerializeSize(block, SER_DISK, CLIENT_VERSION); // 取得区块大小ADD_SERIALIZE_METHODS CBlock::SerializationOp
            CDiskBlockPos blockPos;
            CValidationState state;
    ...
    try {
        ...
        while (!blkdat.eof()) {
            ...
            try {
                ...
                // detect out of order blocks, and store them for later 顺序有问题 hashPrevBlock 
                //必须一个一个的按顺序读盘，并加入
                uint256 hash = block.GetHash();
                ...
                NotifyHeaderTip();
                //对于mapBlockIndex 中没有的数据， 从新根据其中记录的位置取数据
